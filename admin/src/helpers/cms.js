...
export function mapContactSettingsFromApi(content = {}) {
  const recipientEmails = Array.isArray(content.recipientEmails) && content.recipientEmails.length
    ? content.recipientEmails.map((email) => String(email || '').trim()).filter(Boolean)
    : content.recipientEmail
      ? [String(content.recipientEmail).trim()]
      : [''];

  return {
    formTitle: content.formTitle || 'Get A Free Quote',
    recipientEmails,
    plunkFromEmail: content.plunkFromEmail || '',
    plunkFromName: content.plunkFromName || 'GTA Electric Services',
...
export function mapContactSettingsForSave(content = {}) {
  const recipientEmails = (content.recipientEmails || [])
  ...
  return {
    ...
    plunkFromEmail: content.plunkFromEmail,
    plunkFromName: content.plunkFromName,
