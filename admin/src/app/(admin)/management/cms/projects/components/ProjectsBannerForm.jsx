...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapProjectsBannerForSave,
  mapProjectsBannerFromApi,
} from '@/helpers/cms';
...
const ProjectsBannerForm = () => {
  const { showNotification } = useNotificationContext();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState(mapProjectsBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/projects/banner');
      setForm(mapProjectsBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'projects-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/projects/banner', {
        content: mapProjectsBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Page Banner"
      description="Controls the Projects listing page header title and background image."
    >
      {loading ? (
...
export default ProjectsBannerForm;
