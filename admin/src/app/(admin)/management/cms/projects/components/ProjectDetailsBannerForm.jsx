...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapProjectDetailsBannerForSave,
  mapProjectDetailsBannerFromApi,
} from '@/helpers/cms';
...
const ProjectDetailsBannerForm = () => {
  const { showNotification } = useNotificationContext();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState(mapProjectDetailsBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/project-details/banner');
      setForm(mapProjectDetailsBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'project-details-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/project-details/banner', {
        content: mapProjectDetailsBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Project Details Banner"
      description="Default banner for project detail pages. The project title replaces the banner heading when viewing a project."
    >
      {loading ? (
        ...
        <form onSubmit={handleSubmit}>
          <Row className="g-4">
            <Col xs={12}>
              <Form.Group>
                <Form.Label>Default Banner Title</Form.Label>
                <Form.Control
                  value={form.title}
...
export default ProjectDetailsBannerForm;
