...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapServiceAreasBannerForSave,
  mapServiceAreasBannerFromApi,
} from '@/helpers/cms';
...
const ServiceAreasBannerForm = () => {
  const { showNotification } = useNotificationContext();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState(mapServiceAreasBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/service-areas/banner');
      setForm(mapServiceAreasBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'service-areas-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/service-areas/banner', {
        content: mapServiceAreasBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Page Banner"
      description="Controls the Service Areas page header title and background image. Coverage locations are managed under Management → Service Locations, and section copy under Homepage → Coverage."
    >
      {loading ? (
...
export default ServiceAreasBannerForm;
