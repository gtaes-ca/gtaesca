...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapLegalBannerForSave,
  mapLegalBannerFromApi,
} from '@/helpers/cms';
...
const LegalBannerForm = ({ pageKey, pageLabel }) => {
  const { showNotification } = useNotificationContext();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState(mapLegalBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get(`/api/admin/web-content/${pageKey}/banner`);
      setForm(mapLegalBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
        message: e.response?.data?.error || `Failed to load ${pageLabel} banner`,
        variant: 'danger',
      });
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, `${pageKey}-banner-background`);
      }

      const res = await httpClient.put(`/api/admin/web-content/${pageKey}/banner`, {
        content: mapLegalBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
      setForm(mapLegalBannerFromApi(res.data.content || {}));
      showNotification({ message: `${pageLabel} banner saved`, variant: 'success' });
    } catch (err) {
      showNotification({
  ...
  return (
    <ComponentContainerCard
      title={`${pageLabel} Banner`}
      description={`Controls the ${pageLabel} page header title and background image.`}
    >
      {loading ? (
...
export default LegalBannerForm;
