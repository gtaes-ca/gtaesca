...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapServiceDetailsBannerForSave,
  mapServiceDetailsBannerFromApi,
} from '@/helpers/cms';
...
const ServiceDetailsBannerForm = () => {
  const { showNotification } = useNotificationContext();
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState(mapServiceDetailsBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/service-details/banner');
      setForm(mapServiceDetailsBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'service-details-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/service-details/banner', {
        content: mapServiceDetailsBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Service Details Banner"
      description="Default banner for service detail pages. The service name replaces the banner heading when viewing a service."
    >
      {loading ? (
...
export default ServiceDetailsBannerForm;
