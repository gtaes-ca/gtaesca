...
const FaqBannerForm = () => {
  ...
  const [form, setForm] = useState(mapFaqBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/faq/banner');
      setForm(mapFaqBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'faq-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/faq/banner', {
        content: mapFaqBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Page Banner"
      description="Controls the FAQ page header title and background image."
    >
      {loading ? (
