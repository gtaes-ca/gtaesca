...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapContactBannerForSave,
  mapContactBannerFromApi,
} from '@/helpers/cms';
...
const ContactBannerForm = () => {
  ...
  const [form, setForm] = useState(mapContactBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/contact/banner');
      setForm(mapContactBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'contact-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/contact/banner', {
        content: mapContactBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Page Banner"
      description="Controls the Contact page header title and background image."
    >
      {loading ? (
