...
import {
  BANNER_IMAGE_RATIO,
  MAX_CMS_IMAGE_BYTES,
  mapAboutBannerForSave,
  mapAboutBannerFromApi,
} from '@/helpers/cms';
...
const AboutBannerForm = () => {
  ...
  const [form, setForm] = useState(mapAboutBannerFromApi());

  const loadBanner = useCallback(async () => {
    setLoading(true);
    try {
      const res = await httpClient.get('/api/admin/web-content/about/banner');
      setForm(mapAboutBannerFromApi(res.data.content || {}));
    } catch (e) {
      showNotification({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.backgroundImageData) {
        backgroundImage = await uploadBannerImage(form.backgroundImageData, 'about-banner-background');
      }

      const res = await httpClient.put('/api/admin/web-content/about/banner', {
        content: mapAboutBannerForSave({
          ...form,
          backgroundImage,
        }),
      });
  ...
  return (
    <ComponentContainerCard
      title="Page Banner"
      description="Controls the About Us page header title and background image."
    >
      {loading ? (
